...
// Ruta especial de controles táctiles para juegos de estructura combinada
// (ver .claude/skills/add-touch-controls/reference.md): Frogger no separa
// engine.ts + <id>-canvas.tsx, así que en vez de un CanvasHandle expone este
// handle propio con pressKey(code), análogo al pressKey de Snake/Tetris.
...
const FroggerGame = forwardRef<FroggerGameHandle, FroggerGameProps>(
  function FroggerGame(
    ...
    // Traduce un código de tecla a una dirección y la encola como pendingDir,
    // consumida por update() en el próximo tick (ver el switch que antes vivía
    // directo en el listener de teclado). Compartida por handleKeyDown (real)
    // y por pressKey (controles táctiles, ver FroggerGameHandle arriba) para
    // no duplicar el mapeo de teclas ni el guard de game over.
    ...
    useImperativeHandle(ref, () => ({ pressKey: handleAction }), [
      handleAction,
    ]);
    ...
    useEffect(() => {
      ...
      function handleKeyDown(e: KeyboardEvent) {
        if (!NAV_CODES.has(e.code)) return; // P/Esc y demás no se manejan aquí.
        e.preventDefault();
        handleAction(e.code);
      }
        ...
        for (const lane of state.lanes) {
          for (const entity of lane.entities) {
            ...
            if (entity.type === "turtle") {
              const phase =
                (state.elapsedMs + entity.col * 47) % TURTLE_CYCLE_MS;
    ...
    }, [killFrog, resolveArrival, handleAction]);
